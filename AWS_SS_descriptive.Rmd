...
hist(log(saws$SMC), breaks=60)
tail(sort(saws$SMC))
ol <- which(saws$SMC > 5000,)
...
lapply(seq(srh), function(s){
  print(s)
      ...
      scaleFactor <-  max(rhT$RH) / max(rhT$log_SMC_1)
      scaleFactorT <- max(rhT$RH) / max(rhT$Temp)
      ...
      T_P <- ggplot(rhT, aes(x=date))+
            ...
            geom_line(aes(y=RH/ scaleFactorT), color=RHcolor)+
            geom_point(aes(y=RH/scaleFactorT), color=RHcolor, size=1)+
            ...
            scale_y_continuous(
              ...
              # Add a second axis and specify its features
              sec.axis = sec_axis(~.*scaleFactorT, name="RH (%)")
      ...
      gridExtra::grid.arrange(RH_SMC_P, 
                              T_P)
