...
# get_products()
Ldir <- "D:/Surface_Moisture/Landsat_data/"
downloadpathL8 <- paste0(Ldir, "espa_order_downloads_L8/")
downloadpathL7 <- paste0(Ldir, "espa_order_downloads_L7/")
...
#                                products="lsr_landsat_8_c1") # sr and bt and toa 
...
Collection 2 seems to contain only indices, in Col 1 there is surface reflectance (sr), brightness temperature (bt) and top of atmosphere (toa) data available. 
...
```{r, eval=F}
records <- L_records[L_records$level == "bt" | L_records$level == "sr" | L_records$level == "toa",]
...
saveRDS(records, paste0(Ldir, "records.RDS"))
...
L7_records <- get_records(time_range = tr_preStudy,
                               products="lsr_landsat_etm_c1") # sr and bt and toa 
...
```{r, eval=F}
L7_records <- L7_records[L7_records$level == "bt" | L7_records$level == "sr" | L7_records$level == "toa",]
...
ggplot(L7_records, aes(x=start_time, y=cloudcov_land))+
  geom_point()+theme_minimal()+
  ggtitle(paste0("Landsat 8 data Jan 2011 to March 13, n=", nrow(L7_records)/3))
...
L8toa <- L8_records[L8_records$level == "toa",]
L8bt <- L8_records[L8_records$level == "bt",]

L8_rec_list <- list("sr"=L8sr, "toa"=L8toa,"bt"=L8bt)

L7sr <- L7_records[L7_records$level == "sr",]
L7toa <- L7_records[L7_records$level == "toa",]
L7bt <- L7_records[L7_records$level == "bt",]

L7_rec_list <- list("sr"=L7sr, "toa"=L7toa,"bt"=L7bt)
                ...
                lapply(seq(3), function(l){
                    ...
                    dlp <- paste0(downloadpath, names(recordslist)[l], "/")
                      ...
                      print(paste0("starting with ", i))
                        earthexplorer_download(usgs_eros_username=USGSUser,
                                             usgs_eros_password=USGSPW,
                                             output_folder = dlp, 
                                             verbose=T,
...
download_L(downloadpathL8, L8_rec_list)
download_L(downloadpathL7, L7_rec_list)
